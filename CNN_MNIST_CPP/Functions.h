...
image3D reshape_to_3d(const image2D& flattened_array, int output_channels, int kernel_size) {
    image3D result;
    int num_elements = output_channels * kernel_size * kernel_size;
...
image3D reshape_to_3d_conv(const image1D& flattened_input, int height, int width, int output_channels) {
    image3D output(height, image2D(width, image1D(output_channels, 0.0)));

    for (int h = 0; h < height; ++h) {
        for (int w = 0; w < width; ++w) {
            for (int c = 0; c < output_channels; ++c) {
                int flat_index = (h * width + w) * output_channels + c;
                output[h][w][c] = flattened_input[flat_index];
            }
        }
    }
...
image1D read_csv_image1D_conv(const std::string& filename) {
    image1D data;
    std::ifstream file(filename);
