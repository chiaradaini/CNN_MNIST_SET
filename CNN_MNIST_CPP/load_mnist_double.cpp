...
int main() {
    ...
    //     for (auto& row : image) {
    //         for (auto& pixel : row) {
    //             pixel /= normalization_factor; // Divide each pixel value by the normalization factor
    //         }
    //     }
    // }
    ...
    int image_index = rand() % num_images; // Generate a random index
    int num_rows = X_train[image_index].size();
    int num_cols = X_train[image_index][0].size();

    std::cout << "Random image index: " << image_index << std::endl;
